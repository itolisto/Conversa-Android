...
public class ActivityChatWall extends ConversaActivity implements View.OnClickListener, OnMessageClickListener {
	...
	private Runnable isUserTypingRunnable = new Runnable() {
		...
		public void run() {
			...
			SkygearConnection.getInstance().userHasEndedTyping(businessObject.getBusinessId());
			typingFlag = false;
		}
	};
		...
		public void afterTextChanged(Editable s) {
			...
			if (s.toString().isEmpty()) {
				...
				if (!typingFlag) {
					...
					SkygearConnection.getInstance().userHasStartedTyping(businessObject.getBusinessId());
					typingFlag = true;
				}
